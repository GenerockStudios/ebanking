...
# OBJECTIF EXCLUSIF DU PROMPT 8

## Sujet : Module de Search Log et Rapport d'Audit Sécurité
**Acteur Principal** : Administrateur (1) uniquement
**Focus Imposé** : Moteur de recherche multicritère ultra poussé sur `journal_audit` (par intervalle, utilisateur, types d'échecs). Génération d'un export PDF (impression) format officiel, filigrané, classé 'Strictement Confidentiel' pour l'inspection.
