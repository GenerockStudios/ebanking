...
# OBJECTIF EXCLUSIF DU PROMPT 5

## Sujet : Procès Verbal d'Arrêté et Clôture de Caisse
**Acteur Principal** : Caissier (3) ou Superviseur/Admin (1)
**Focus Imposé** : Exploiter `sessions_caisse`. Comparer les agrégats de la table `transactions` de la journée avec `solde_initial_caisse` pour déduire le `solde_final_systeme`. Interface affichant les écarts et générant un PV d'Arrêté à imprimer et faire signer.
