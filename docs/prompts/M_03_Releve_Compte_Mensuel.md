...
# OBJECTIF EXCLUSIF DU PROMPT 3

## Sujet : Relevé de Compte Détaillé avec Filtrage Complexe
**Acteur Principal** : Caissier (3) & Administrateur (1)
**Focus Imposé** : Croiser `transactions` et `comptes` pour un tableau dynamique avec sélecteur de dates. Calculer le 'Solde Avant Période' mathématiquement sans l'avoir en DB, et le 'Solde Après', avec impression d'Extrait de Compte type banque traditionnelle.
